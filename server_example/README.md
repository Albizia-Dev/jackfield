...
Успех даёт HTTP 204 и отправляет FCM data message `type=end`, `reason=remote`. Повторное завершение уже завершённого звонка даёт 204 без повторной отправки. Ошибка FCM даёт 502 и допускает повтор запроса.
