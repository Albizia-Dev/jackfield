...
An answer deadline is stored with its action receipt. The worker checks it with a timer while running and recovers it on activation, initialization, and supported Periodic Background Sync wakes. It does not register one-shot Background Sync for the deadline: browsers may fire that event before the deadline and repeatedly wake the worker. No browser guarantees an exact deadline wake while the worker is suspended; the next permitted wake reconciles the call.
