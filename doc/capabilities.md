...
При отказе разрешений или хранилища адаптер сообщает `unavailable` либо типизированный отказ команды. Android `nativeCallUi` означает self-managed Telecom, а не экран стандартной телефонной программы. Web Notification не является системным call UI. В 0.0.1 `pubspec.yaml` регистрирует только Android, iOS, macOS и Web; Windows/Linux исходники остаются незарегистрированным scaffold. [CI](ci.md) проверяет реализованные адаптеры и отдельно scaffold status Windows/Linux. Проверки исходников/симулятора не заменяют [ручную матрицу](validation-matrix.md).
